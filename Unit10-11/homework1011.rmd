...
## Proportion Conceptual questions
  ...
  1.  State under what circumstance that a difference in proportion confidence interval or test should not be used in favor of an odds ratio metric.
      ...
      If the data is sampled retrospectively, then only the odds ratio can be estimated.
  ...
  2.  Under what sampling schemes can a hypothesis test be generally worded as a "test for association", rather than a test for difference in proportion, odds, or relative risk?
      ...
      In a double-blind study, such as the vitamin C example from the book.
  ...
  3.  It might be tempting in a predictive model setting (using a test set) to obtain  a count of correct vs incorrect prediction for two or more modeling approaches.  To be clear here, imagine that the classification accuracy on a test set of 200 observations for model 1 is .82 while for model 2 it was .80.  To determine that one model is actually predicting "better"" than the other in terms of overall accuracy, you produce a confidence interval for the difference in proportion of correctly predicted observations and the interval does not contain 0. Why is this type of analysis using a two by two count table inappropriate?
      ...
      This is an apples to oranges comparison, different modeling approaches are fundamentally different and as an example one modeling approach may be better suited to one thing than the other, or one might be more sensitive to different data types.  
  ...
  4.  Give an example of study that could not be conducted in a prospective manner due to either logistical or ethical considerations.
      ...
      If for example we wanted to study if eating batteries causes cancer, so we assign one group of subjects to eat batteries and the other as a           control.
  ...
  5.  What metric allows for the following interpretation: "The chances of a person getting the flu who has not taken the flu vaccine is thirty percent (1.3 times) higher than someone who has taken the flu vaccine"?
      ...
      This is a retrospective study and the metric is an odds ratio.
...
#this is the proportion of people who got the cold
...
Based on the above output, I believe the ratio is for odds (N:Y) and that the placebo is on the top.
...
The version we are after is (mymat,rev="columns")
...
Based on the above output, I believe the ratio is for odds (N:Y) and that the placebo is on the top, so we are looking at the "No" proportion calcukating relative risk. And in response to the second step, The version we are after is (mymat,rev="columns").
...
(A) Which factor is the response?
    In a retrospective study subjects are selected from groups with specified response levels.  Their explanatory factors are then determined.
    In this study the presence of breast cancer is the response.
    
(B) Is this a retrospective or prospective study?
    This is a retrospective study as we are studying the habits of women and drinking.  We will consider this infomation about their past.
    
(C) Is this a randomized experiment or observational study?
    This is an observational study since we have selected two different groups of women to study, and have NOT placed them into treatment groups.
    
```{r}
mymat<-matrix(c(330,658,204,386),2,2,byrow=T,dimnames=list(c("Fewer than four drinks per week ","4 or more drinks per week"),c("Cases","Controls")))
mymat
...
Step 2: ${\alpha}=0.05$
Step 3: zstat
Step 4: p-value: 0.6601
Step 5: FTR
Step 6: Conclusion: We will fail to reject the null hypothesis and conclude that there isn't a signigicant difference between the two groups. 
Step 7: Since this is an observational experiment, we cannot generalize this to the greater population.  We also failed to reject the null hypothesis, and concluded that there isn't a difference in breast cancer rates among women who drink and those who do not drink.



## Exercise #2  Mantel-Haenszel Tests- Don't do this!
...
Answer: N/A Professor Turners wall post explained not to do the MH problems. 
