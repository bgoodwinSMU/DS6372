...
1.  State the assumptions of the MANOVA and LDA models.
    The typical assumptions of ANOVA should be checked:
    Normality, Equality of variance, and univariate outliers
    In addition to the ANOVA assumptions MANOVA requires the following:
    1) Absence of multivariate outliers
    2) Linearity
    3) Absence of multicollinearity 
    4) Equality of covariance matrices
    
    The assumptions of LDA are the following:
    1) All of the assumptions of MANOVA, and ANOVA
    2) The data is normally distributed
    3) The data has equal variances



2.  What is the fundamental difference between MANOVA and LDA since they both share the same underlying assumptions?

    I think there are some good differences between MANOVA and LDA.  MANOVA is really an extension of ANOVA, and LDA is somewhat similar to     PCA.In LDA the QOI's are much different beginning with is it possible to differentiate among groups on the basis of differences in a set of shared variables?
    Which variables are most important in differentiating among groups?
    What are the chances that the observed differences are not due to random variation?
    In addition, it is worth noting that while LDA has a theoretical parametric basis 

3.  What is a confusion matrix? 

    Simply, a confusion matrix is a visual way to evaluate the performance of specific algorithms.  An NxN matrix evaluates the                 classifcation model, where N is the number of target classes, and the matrix plots the actual target values against those predicted.  

4.  What is the difference between LDA and QDA, that is, what assumption of the LDA model is relaxed?

    I like to think of the difference between these as the shape of the decision line.  In LDA this line is linear and in QDA this line is      non-linear.  The assumption of LDA that is relaxed is allowing different feature covariance matrices for different classes, and thus        leads to the quartic boundary.
...
# construct the LDA model
mylda <- lda(Response ~ X1 + X2, data = full,prior=c(0.8,0.2)/1)
# draw discrimination line
np <- 300
nd.x <- seq(from = min(full$X1), to = max(full$X1), length.out = np)
nd.y <- seq(from = min(full$X2), to = max(full$X2), length.out = np)
...
prop = mylda$svd^2/sum(mylda$svd^2)
prop

```
Looks like the boundary has shifted considerably, which is to be expected as we know that the prior probability of a "yes" response is 0.8.
...
Based on the plot above, we can see that the addition of this point can be considered an outlier.  And this point certainly changes the decision boundary.
...
```
Yes, it appears that LDA easily suffers from overfitting.
